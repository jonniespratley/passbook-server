...
module.exports = function(config, app) {
	...
	router.route( '/passes').all(function(req, res, next) {
	...
	router.get( '/' + 'cmd' + '/' + ':command', function(req, res) {
		var results = {}, child;
	...
	router.post( '/devices/:deviceLibraryIdentifier/registrations/:passTypeIdentifier/:serialNumber?', jsonParser, registerPass);
	router.post( '/v1/devices/:deviceLibraryIdentifier/registrations/:passTypeIdentifier/:serialNumber?', jsonParser, registerPass);

	//Unregister Pass on device
	router.delete( '/devices/:deviceLibraryIdentifier/registrations/:passTypeIdentifier/:serialNumber', unregisterPass);
	router.delete( '/v1/devices/:deviceLibraryIdentifier/registrations/:passTypeIdentifier/:serialNumber', unregisterPass);
	...
	router.get( '/devices/:deviceLibraryIdentifier/registrations/:passTypeIdentifier', function(req, res, next) {
	...
	router.get( '/passes/:passTypeIdentifier/:serialNumber?', function(req, res, next) {
		var obj = {};
	...
	//Get latest version of pass
	router.get('/passes/:id?', function(req, res, next) {
		var id = req.params.id;
		if(!id){
			err = { error: 'Must provide ID' };
			res.status(400).send(err);
		}
		rest.fetch('passes', id).then(function(data) {
			res.status(200).send(data);
		}, function(err) {
			res.status(400).send(err);
		});
	...
	//Send push to device
	router.get('/push/:token', function(req, res, rext) {
		console.log('Push to device ' + req.param('token'));
	});

	//I am the signpass route
	router.get( '/passes/:id/sign', function(req, res, next) {
	...
	router.get('/passes/:id/export', function(req, res) {
...
// mount the router on the app
	...
	router.use(function (req, res, next) {
		...
		console.log(
			chalk.yellow('[jps-passbook-routes] -', req.method, req.url, JSON.stringify(req.query), req.get('Authorization'), JSON.stringify(req.body))
		);
		next();
	});
