	...
	var RestResource = {
		...
		connect : function() {
			...
			MongoClient.connect(config.database.url, function(err, db) {
				console.log('Trying to connect to', config.database.url);
				self.db = db;
				if (!err) {
					self.log('Connected to ', config.database.url);
					defer.resolve(db);
				} else {
					console.error('Could not connect to ', config.database.url);
					defer.reject('Could not connect to ', config.database.url);
				}
			});
